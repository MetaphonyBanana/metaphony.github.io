...
// カメラが(ユーザー操作で)動いたら追加表示を元に戻す。
// なお軸ステーションへのフライト到達直後にも一度だけcontrols.update()が呼ばれ'change'が発火するが、
// そのタイミングでは(このコールバックの実行順が先になるため)まだyOverlayActive/xOverlayActiveがfalseなので無害。
...
if (starHit) {
  ...
  if (axisStationOverlay.yEndDot.visible) {
    const endHit = raycaster.intersectObject(axisStationOverlay.yEndDot)[0];
    if (endHit) {
  ...
  if (axisStationOverlay.xEndDot.visible) {
    const xEndHit = raycaster.intersectObject(axisStationOverlay.xEndDot)[0];
    if (xEndHit) {
  ...
  if (axisStationOverlay.xOriginDot.visible) {
    const originHit = raycaster.intersectObject(axisStationOverlay.xOriginDot)[0];
    if (originHit) {
...
// ── YZパネルのホバー処理(マウスを乗せると反射ガラスとして浮かび上がる) ──
renderer.domElement.addEventListener('pointermove', (e) => {
  if (!yzPanel.isActive() || cameraBusy || intro.getState() !== 'home') {
    if (yzPanel.isHovered()) yzPanel.setHovered(false);
  ...
  const panelHit = raycaster.intersectObject(yzPanel.mesh)[0];
  yzPanel.setHovered(!!panelHit);
  renderer.domElement.style.cursor = panelHit ? 'pointer' : 'default';
});
